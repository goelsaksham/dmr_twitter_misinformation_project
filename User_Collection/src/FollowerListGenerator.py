...
def generate_followers_list(user_id):
    """
    :param user_id: User ID of some twitter user (Numeric)
    :return: A list of user id's corresponding to the users which follow the user corresponding to the user id passed as
    the parameter to the function.
    """
    api = get_api_auth()
    follower_user_ids = []
    for page_fr in tweepy.Cursor(api.followers_ids, user_id=user_id, count=5000).pages():
        follower_user_ids.extend(page_fr)
        time.sleep(5)
...
def main(user_id):
    try:
        print(generate_followers_list(user_id))
    except:
